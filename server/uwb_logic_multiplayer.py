...
max_uncertainity = 0.1       # Maximum Uncertainity Radius
    ...
    while True:
        ...
        try:
            ...
            if len(tags[tag_number-1].used_anchors_index) >= 3: # at least 3 anchors to calculate position
                ...
                # Web socket to headset - Sending data without caring for the uncertainity
            ...
            sio.emit(event ='tag_update',data= {"tag_n" : 1, "status" : False})
            sio.emit(event ='tag_update',data= {"tag_n" : 2, "status" : False})
